...
d3.json("https://raw.githubusercontent.com/swoldetsadick/Spark/master/data1.json", function(error, classes) {
  if (error) throw error;

  var nodes = cluster.nodes(packageHierarchy(classes)),
      links = packageImports(nodes);
      console.log(link.data(bundle(links)))
  link = link.data(bundle(links)).enter().append("path").each(function(d){d.source = d[0], d.target = d[d.length - 1];}).attr("class", "link").attr("d", line);
console.log("done");
  node = node
